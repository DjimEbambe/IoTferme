#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>

#include "config.h"
#include "cJSON.h"
#include "esp_event.h"
#include "esp_log.h"
#include "esp_netif.h"
#include "esp_now.h"
#include "esp_wifi.h"
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
#include "light_adc.h"
#include "mq135_adc.h"
#include "nvs_store.h"
#include "pins.h"
#include "relays.h"
#include "sht41.h"
#include "time_sync.h"
#include "config_cli.h"
#include "espnow_init.h"
#include "frames.h"
#include "nvs_flash.h"

static const char *TAG = "sensor";
static uint8_t s_gateway_mac[ESP_NOW_ETH_ALEN] = {0};
static const uint8_t s_broadcast_mac[ESP_NOW_ETH_ALEN] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
static bool s_gateway_known = false;

static void iso_timestamp(char *buffer, size_t len)
{
    int64_t epoch_ms = time_sync_now_ms();
    time_t sec = (time_t)(epoch_ms / 1000);
    struct tm tm_utc = {0};
    gmtime_r(&sec, &tm_utc);
    int ms = (int)(epoch_ms % 1000);
    snprintf(buffer, len, "%04d-%02d-%02dT%02d:%02d:%02d.%03dZ",
             tm_utc.tm_year + 1900, tm_utc.tm_mon + 1, tm_utc.tm_mday,
             tm_utc.tm_hour, tm_utc.tm_min, tm_utc.tm_sec, ms);
}

static esp_err_t ensure_peer(void)
{
    if (!s_gateway_known) {
        return ESP_FAIL;
    }
    esp_now_peer_info_t info = {
        .channel = EDGE_ESPNOW_CHANNEL,
        .ifidx = WIFI_IF_STA,
        .encrypt = true,
    };
    memcpy(info.peer_addr, s_gateway_mac, ESP_NOW_ETH_ALEN);
    memcpy(info.lmk, EDGE_ESPNOW_LMK, strnlen(EDGE_ESPNOW_LMK, ESP_NOW_KEY_LEN));
    if (!esp_now_is_peer_exist(s_gateway_mac)) {
        return esp_now_add_peer(&info);
    }
    return ESP_OK;
}

static esp_err_t send_to_gateway(const uint8_t *payload, size_t len)
{
    bool encrypt = s_gateway_known;
    const uint8_t *target = s_gateway_known ? s_gateway_mac : s_broadcast_mac;
    return espnow_send(target, payload, len, encrypt);
}

static void publish_ack(const char *corr, bool ok, const char *message)
{
    cJSON *root = cJSON_CreateObject();
    cJSON_AddStringToObject(root, "type", "ack");
    cJSON_AddStringToObject(root, "asset_id", EDGE_DEVICE_ASSET_ENV);
    cJSON_AddStringToObject(root, "correlation_id", corr ? corr : "n/a");
    cJSON_AddBoolToObject(root, "ok", ok);
    if (message) {
        cJSON_AddStringToObject(root, "message", message);
    }
    char ts[32];
    iso_timestamp(ts, sizeof(ts));
    cJSON_AddStringToObject(root, "ts", ts);
    char *json = cJSON_PrintUnformatted(root);
    if (json) {
        send_to_gateway((const uint8_t *)json, strlen(json));
        cJSON_free(json);
    }
    cJSON_Delete(root);
}

static relay_channel_t relay_from_key(const char *key)
{
    if (strcmp(key, "lamp") == 0) {
        return RELAY_CHANNEL_LAMP;
    }
    if (strcmp(key, "fan") == 0) {
        return RELAY_CHANNEL_FAN;
    }
    if (strcmp(key, "heater") == 0) {
        return RELAY_CHANNEL_HEATER;
    }
    if (strcmp(key, "door") == 0) {
        return RELAY_CHANNEL_DOOR;
    }
    return RELAY_CHANNEL_EXTRA1;
}

static void handle_command(const uint8_t *data, size_t len)
{
    char *buffer = strndup((const char *)data, len);
    if (!buffer) {
        return;
    }
    cJSON *root = cJSON_Parse(buffer);
    free(buffer);
    if (!root) {
        return;
    }
    const cJSON *corr = cJSON_GetObjectItem(root, "correlation_id");
    const char *corr_str = cJSON_IsString(corr) ? corr->valuestring : "n/a";
    bool ok = true;
    const cJSON *relay = cJSON_GetObjectItem(root, "relay");
    if (cJSON_IsObject(relay)) {
        const cJSON *entry = NULL;
        cJSON_ArrayForEach(entry, relay) {
            relay_channel_t ch = relay_from_key(entry->string);
            bool on = strcmp(cJSON_GetStringValue(entry), "ON") == 0;
            relays_set(ch, on);
        }
    }
    publish_ack(corr_str, ok, ok ? "applied" : "error");
    cJSON_Delete(root);
}

static void handle_control_message(const uint8_t *data, size_t len)
{
    char *buffer = strndup((const char *)data, len);
    if (!buffer) {
        return;
    }
    cJSON *root = cJSON_Parse(buffer);
    free(buffer);
    if (!root) {
        return;
    }
    const cJSON *type = cJSON_GetObjectItem(root, "type");
    if (cJSON_IsString(type) && strcmp(type->valuestring, "cmd") == 0) {
        handle_command(data, len);
    } else if (cJSON_IsString(type) && strcmp(type->valuestring, "time_sync") == 0) {
        time_sync_handle_frame(data, len);
    } else if (cJSON_IsString(type) && strcmp(type->valuestring, "pair_ack") == 0) {
        const cJSON *mac = cJSON_GetObjectItem(root, "gateway_mac");
        if (cJSON_IsString(mac) && strlen(mac->valuestring) == 17) {
            sscanf(mac->valuestring, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
                   &s_gateway_mac[0], &s_gateway_mac[1], &s_gateway_mac[2],
                   &s_gateway_mac[3], &s_gateway_mac[4], &s_gateway_mac[5]);
            s_gateway_known = true;
            ensure_peer();
            ESP_LOGI(TAG, "paired with gateway %s", mac->valuestring);
        }
    }
    cJSON_Delete(root);
}

static void espnow_recv_cb(const esp_now_recv_info_t *info, const uint8_t *data, int len)
{
    if (len <= 0) {
        return;
    }
    handle_control_message(data, len);
}

static void telemetry_task(void *arg)
{
    (void)arg;
    const TickType_t interval = pdMS_TO_TICKS(EDGE_TELEMETRY_INTERVAL_NORMAL_MS);
    TickType_t last_wake = xTaskGetTickCount();
    light_sample_t light = {0};
    sht41_sample_t sht = {0};
    mq135_sample_t mq = {0};
    bool warmup = true;
    uint32_t warmup_ticks = pdMS_TO_TICKS(120000);
    TickType_t start_tick = xTaskGetTickCount();

    while (true) {
        sht41_read(&sht);
        light_sensor_read(&light);
        mq135_read(sht.temperature_c, sht.humidity_pct, &mq);
        if (warmup && (xTaskGetTickCount() - start_tick) > warmup_ticks) {
            warmup = false;
        }
        cJSON *root = cJSON_CreateObject();
        cJSON_AddStringToObject(root, "type", "telemetry");
        cJSON_AddStringToObject(root, "site", EDGE_SITE_ID);
        cJSON_AddStringToObject(root, "device", EDGE_DEVICE_ID);
        cJSON_AddStringToObject(root, "asset_id", EDGE_DEVICE_ASSET_ENV);
        cJSON_AddStringToObject(root, "channel", "env");
        cJSON *metrics = cJSON_CreateObject();
        cJSON_AddNumberToObject(metrics, "t_c", sht.temperature_c);
        cJSON_AddNumberToObject(metrics, "rh", sht.humidity_pct);
        cJSON_AddNumberToObject(metrics, "lux", light.lux);
        cJSON_AddNumberToObject(metrics, "mq135_ppm", warmup ? 0 : mq.ppm);
        cJSON_AddItemToObject(root, "metrics", metrics);
        char ts[32];
        iso_timestamp(ts, sizeof(ts));
        cJSON_AddStringToObject(root, "ts", ts);
        char msg_id[EDGE_MSG_ID_LENGTH] = {0};
        frames_make_msg_id(msg_id, sizeof(msg_id));
        cJSON_AddStringToObject(root, "msg_id", msg_id);
        cJSON_AddStringToObject(root, "idempotency_key", msg_id);
        char *json = cJSON_PrintUnformatted(root);
        if (json) {
            send_to_gateway((const uint8_t *)json, strlen(json));
            cJSON_free(json);
        }
        cJSON_Delete(root);
        vTaskDelayUntil(&last_wake, interval);
    }
}

static void init_wifi(void)
{
    wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
    ESP_ERROR_CHECK(esp_netif_init());
    ESP_ERROR_CHECK(esp_event_loop_create_default());
    esp_netif_create_default_wifi_sta();
    ESP_ERROR_CHECK(esp_wifi_init(&cfg));
    ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_STA));
    ESP_ERROR_CHECK(esp_wifi_start());
    ESP_ERROR_CHECK(esp_wifi_set_channel(EDGE_ESPNOW_CHANNEL, WIFI_SECOND_CHAN_NONE));
}

void app_main(void)
{
    ESP_ERROR_CHECK(nvs_flash_init());
    nvs_store_init("device");
    time_sync_init();
    relays_init();
    relays_all_off();
    sht41_init();
    light_sensor_init();
    mq135_init(10.0f);
    config_cli_start();

    init_wifi();
    uint8_t pmk[ESP_NOW_KEY_LEN] = {0};
    memcpy(pmk, EDGE_ESPNOW_PMK, strnlen(EDGE_ESPNOW_PMK, ESP_NOW_KEY_LEN));
    uint8_t lmk[ESP_NOW_KEY_LEN] = {0};
    memcpy(lmk, EDGE_ESPNOW_LMK, strnlen(EDGE_ESPNOW_LMK, ESP_NOW_KEY_LEN));
    ESP_ERROR_CHECK(espnow_setup(pmk, lmk, EDGE_ESPNOW_CHANNEL));
    ESP_ERROR_CHECK(esp_now_register_recv_cb(espnow_recv_cb));

    xTaskCreate(telemetry_task, "telemetry", 4096, NULL, 5, NULL);
    ESP_LOGI(TAG, "device sensor started");
}
